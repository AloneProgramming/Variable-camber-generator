...
int main() {
    ...
    cout << "Enter angle of deflection (deg, + is up): ";
    cin >> def_angle_deg;
    ...
    for (int i = 0; i <= num_points; ++i) {
        ...
        if (x > hinge && def_angle_deg != 0) {
            ...
            double alpha = total_deflection_rad * (x - hinge) / (1 - hinge);
            ...
            if (x >= trimmer) {
                ...
                double alpha_hinge = total_deflection_rad * (trimmer - hinge) / (1 - hinge);
                double cos_alpha_hinge = cos(alpha_hinge);
                double sin_alpha_hinge = sin(alpha_hinge);
                
                double trimmer_hinged = hinge + (trimmer - hinge) * cos_alpha_hinge - (trimmer_yc - hinge_yc) * sin_alpha_hinge;
                double trimmer_yc_hinged = hinge_yc + (trimmer - hinge) * sin_alpha_hinge + (trimmer_yc - hinge_yc) * cos_alpha_hinge;
            
                double alpha_trimm = -1.0 * (total_deflection_rad * (trimmer - hinge) / (1 - hinge)) * ((x - trimmer) / (1 - trimmer));
    ...
    cout << "NACA" << m << p << xx << ", hinge = " << hinge << ", def angle = " << def_angle_deg << ", trimmer = " << trimmer << endl;
